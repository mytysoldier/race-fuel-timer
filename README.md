...
`make lint` は Swift コンパイラの構文解析でソースの基本チェックを行います。`typecheck` はテスト用ビルド、`test` は iPhone 17 Pro Simulator 上の単体テスト、`build` は Simulator 向けアプリビルドです。品質ゲートはすべてローカルで実行します。
