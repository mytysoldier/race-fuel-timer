...
- 給水／補給を区別する通知文とiOS標準の通知音を設定する。音・振動の再生可否とパターンはiOSの通知設定に従う
...
前面、別アプリ前面、画面ロック中の配信はiOSに委ねる。サイレントモード、Focus、音量、ユーザーの通知設定による音・振動・表示は保証しない。Critical Alert、動的TTS、バックグラウンドGPS計測は使用しない。iOS実機での通知音・振動・ロック画面表示の確認は #16 で記録する。
